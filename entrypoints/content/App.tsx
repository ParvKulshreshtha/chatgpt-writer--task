...
interface AppProps {
  insertMsg: (message: string) => void;
}


const generatedMsg = "Thank you for the opportunity! If you have any more questions or if there's anything else I can help you with, feel free to ask."

const App: React.FC<AppProps> = ({insertMsg}) => {
  ...
  return (
    <div>
      ...
      {isModalOpen && (
        <div contentEditable={false} style={modalStyle} onClick={e=>setIsModalOpen(!isModalOpen)}>
  <div style={modalContentStyle} onClick={e=>e.stopPropagation()}>
    {/* Chat section */}
    {showChat !== "" && <div style={{ display: "flex", flexDirection: "column", gap: "10px", maxHeight: "300px", overflowY: "auto", marginBottom: "20px" }}>
      {/* Message from us */}
      <div style={{ alignSelf: "flex-end", backgroundColor: "#E3F2FD", padding: "10px", borderRadius: "10px", maxWidth: "70%", color: "#333" }}>
        {showChat}
      </div>
      
      {/* Message from other end */}
      <div style={{ alignSelf: "flex-start", backgroundColor: "#F0F0F0", padding: "10px", borderRadius: "10px", maxWidth: "70%", color: "#333" }}>
      {generatedMsg}
      </div>
    </div>}
    ...
    >
      {showChat && (
        <button
          ...
          onClick={() => insertMsg(generatedMsg)}
        >
          Insert
        </button>
      )}
